...
Returns a compact, summary-friendly payload — not raw rows. The llm-gateway uses
...
async def read_tab_execute(args: dict[str, Any]) -> dict[str, Any]:
    ...
    # block the llm-gateway loop's event loop.
...
def _read_tab_sync(project_id: str, tab: str, limit: int) -> dict[str, Any]:
    ...
    # consent-assessment) — return the project header so the llm-gateway has *some*

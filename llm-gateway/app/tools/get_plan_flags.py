...
severities, quotes, page refs so the llm-gateway can answer detailed questions
("what fire-safety flags are on plan X?").

For large plans (more than ``MAX_INLINE_FLAGS`` flags, e.g. Parliament-scale
sets with thousands of findings) this tool refuses to return the full set
and tells the llm-gateway to use ``list_plan_flags`` with filters instead.
That prevents blowing the model's context window with a single tool call.
"""
...
def get_plan_flags_schema() -> dict[str, Any]:
    return {
        ...
        "description": (
            ...
            "flags, categories, or severities. For large uploads with more "
            f"than {MAX_INLINE_FLAGS} flags this returns an error pointing you "
            "to list_plan_flags — use that with filters (severity, sheet "
            "range, etc.) instead of asking for everything at once."
        ),
        "parameters": {
            "type": "object",
            "properties": {
...
def _sync(upload_id: str) -> dict[str, Any]:
    ...
    for table, kind, extra in (
        ...
        if row and row.data:
            ...
            if flag_count > MAX_INLINE_FLAGS:
                return {
                    "id": data["id"],
                    "kind": kind,
                    ...
                    "error": "too_many_flags",
                    "hint": (
                        f"This upload has {flag_count} flags (cap "
                        f"{MAX_INLINE_FLAGS}). Use list_plan_flags with "
                        "filters (severity, sheet_from/sheet_to, "
                        "discipline) to page through them, or "
                        "get_plan_flag for a specific one."
                    ),
                }

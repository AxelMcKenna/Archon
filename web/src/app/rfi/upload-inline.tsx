...
export function UploadInline({ projects }: { projects: Project[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/rfi?letter=${res.letter_id}`);
      router.refresh();
    } catch (err) {
  ...
  if (!projects.length) {
    return (
      <div className="rounded-lg border border-ink-700/10 p-5 text-sm">
        <p className="mb-2">You need a project before you can upload an RFI letter.</p>
        <Link href="/projects/new" className="rounded-lg bg-ink-900 text-white px-4 py-2 text-xs font-medium inline-block">
          Create a project
        </Link>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <button
        disabled={!file || busy}
        className="rounded-lg bg-ink-900 text-white px-4 py-2 text-sm font-medium disabled:opacity-50 sm:self-end"
      >

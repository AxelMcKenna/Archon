...
> **Key signal for ATLAS**: Selwyn produces three RFI subtypes that should be classified separately:
...
> **Key signal for ATLAS**: An Engineering Acceptance RFI lives outside the BCA RFI flow. If a customer is in the urban multi-unit pathway, they will likely receive RFIs from **two separate council processes** with **two separate clocks**.

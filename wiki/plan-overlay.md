...
Source: `api/app/plans/bbox.py::attach_page_bbox`
...
Source: `api/app/vision/plans/vision_pass.py::verify_flags`
...
Source: `api/app/plans/bbox_refiner.py`
...
Source: `api/app/plans/ocr_refiner.py`
...
- **Self-consistency voting** — `analyse_plan` runs the vision pass
  ...
  bucket. Implementation: `vote_flags()` in `api/app/plans/vote.py` and
  `run_single_vision_pass()` in `api/app/vision/plans/vision_pass.py`.
  Set `PLAN_ANALYSER_VOTING_N=1` to disable voting (cheap dev mode).
...
```
api/app/
├── plans/                       # plan-analyser orchestration + non-vision logic
│   ├── analyzer.py              # top-level analyse_plan: doc-rules → vision → verify → bbox → OCR
│   ├── vote.py                  # self-consistency voting across N runs
│   ├── bbox.py                  # tile-local → page-relative bbox normalisation
│   ├── bbox_refiner.py          # text-layer fuzzy-match → snap bbox (Phase F)
│   ├── ocr_refiner.py           # RapidOCR fallback for vectorised labels (Phase G)
│   ├── overlay.py               # Pillow renderer (page PNGs + multi-page PDF)
│   ├── flags_store.py           # plan_flags table writes
│   └── prompt.py                # taxonomy block for the analyser prompt
├── vision/                      # vision engine (shared infra + per-pipeline)
│   ├── core/
│   │   ├── renderer.py          # PDF → PNG, adaptive DPI, 2x2 tiling
│   │   ├── prompts.py           # markdown prompt loader (frontmatter + {{var}})
│   │   └── invoker.py           # Gemini vs OpenRouter dispatch
│   └── plans/
│       ├── schema.py            # analyser + verifier tool schemas + prompt filenames
│       └── vision_pass.py       # run_single_vision_pass + verify_flags (the LLM calls)
├── llm/
...
└── prompts/
    ...
    └── plan_verification_v2.md  # verifier prompt
